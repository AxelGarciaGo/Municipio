        ...
        if($cambio>=0){
            ...
            if($consulta){
                if($vacio=mysqli_fetch_row($consulta)>0){
                    $sql="INSERT INTO festividades(num_toma, anio, cantidad, tipo, pagado, cambio, fecha) VALUES 
                    ('$_REQUEST[numToma]','$_REQUEST[anio]','$_REQUEST[cantidad]','$_REQUEST[festividad]','$_REQUEST[recibido]','$cambio','$fecha')";
                    // Validación de consulta
                    $consulta=mysqli_query($conexion,$sql);
                    // Condición para revisar si se hizo o no la consulta
                    if($consulta){
                        // Redireccionamiento de la pagina
                        header("Location:festividadesRecibo.php?var_pin=".($num));
                    }else{
...
<html>
    ...
    <body>
        ...
        a las festividades -->
        <li class="dropdown">
            <a href="javascript:void(0)" class="active">Festividades</a>
            <div class="dropdown-content">
                <!-- Menú desplegable -->
        ...
        <div align="center">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'])?>" method="post">
                ...
                <label>Festividad</label><br>
                <select class="input" name="festividad" >
                    <option value="independencia">Independecia</option>
